...
void main() async {
  ...
  try {
    ...
    print('--- SYNCING DB IMAGEPATHS TO ACTUAL STORAGE IDs ---');
    ...
    for (var collection in ['popular_services', 'services']) {
      ...
      for (var doc in result.documents) {
        ...
        // Find a file that matches the title or ID
        var file = files.where((f) => f.name.toLowerCase().contains(title.toLowerCase().split(' ').first)).firstOrNull;

        if (file != null) {
          if (doc.data['imagePath'] != file.$id) {
            print('Collection $collection | Doc: $title | Updating imagePath: ${doc.data['imagePath']} -> ${file.$id}');
            await databases.updateDocument(
              databaseId: 'database-default',
              collectionId: collection,
              documentId: doc.$id,
              data: {'imagePath': file.$id},
            );
          }
        }
      }
